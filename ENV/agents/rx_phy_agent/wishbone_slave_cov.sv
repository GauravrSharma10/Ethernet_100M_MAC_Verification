...
covergroup wb_cg;
  ...
  // Crosses automatically update when .sample() is called
...
virtual function void write(wishbone_seq_item t);
  ...
  // Handling the dynamic queue (data_q)
  if (t.data_q.size() == 0) begin
     // Optional: sample once with dummy data if queue is empty
     this.sample_data = 0;
     wb_cg.sample();
  end else begin
    foreach (t.data_q[i]) begin
      this.sample_data = t.data_q[i];
      // This triggers sampling for ALL coverpoints and crosses
      wb_cg.sample(); 
    end
  end
endfunction
